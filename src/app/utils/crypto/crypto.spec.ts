...
describe('crypto', () => {
  ...
  it('test sign and verify with SHA-256 and ECDSA', (done: DoneFn) => {
    ...
    createEcKeyPair$()
      .pipe(
        switchMap(keyPair =>
          zip(of(keyPair), signWithSha256AndEcdsa$(message, keyPair.privateKey))
        ),
        switchMap(([keyPair, signatureHex]) =>
          verifyWithSha256AndEcdsa$(message, signatureHex, keyPair.publicKey)
        )
      )
      .subscribe(result => {
